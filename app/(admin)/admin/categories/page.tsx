...
export default function CategoryManagement() {
  ...
  const categories = (Object.keys(CATEGORY_LABELS) as ProductCategory[]).map(slug => ({
    id: slug,
    slug,
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage product categories available on the platform.</p>
        </div>
        <button
      ...
      {isAdding && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-emerald-100 animate-in slide-in-from-top-4">
          <h2 className="text-lg font-semibold mb-4 text-gray-900">New Category</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {categories.map((category) => (
          <div key={category.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4 group hover:border-emerald-200 transition-colors">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-gray-500 font-mono truncate">{category.slug}</div>
            </div>
            <button 
