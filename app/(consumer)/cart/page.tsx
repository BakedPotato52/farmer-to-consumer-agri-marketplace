    ...
    return (
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-16 flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-8 max-w-md text-center">
          Looks like you haven't added any fresh produce to your cart yet. Let's fix that!
        </p>
        <Link 
          href="/products" 
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <div className="flex-1 space-y-8">
          {Object.entries(groupedItems).map(([farmerId, group]) => (
            <div key={farmerId} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                  ...
                  <div key={item.productId} className="p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <Link href={`/products/${item.productId}`} className="font-bold text-gray-900 dark:text-white hover:text-emerald-600 transition-colors">
                          {item.productName}
                        </Link>
                        {item.isOrganic && (
                          <span className="bg-green-100 text-green-700 text-[10px] font-bold px-1.5 py-0.5 rounded uppercase">Organic</span>
                        )}
                      </div>
          ...
          <div className="flex justify-between items-center">
            <Link href="/products" className="text-emerald-600 font-medium hover:underline flex items-center gap-1 text-sm">
              ← Continue Shopping
            </Link>
            <button 
              onClick={clearCart}
        ...
        <div className="w-full lg:w-96 shrink-0">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 sticky top-24 overflow-hidden">
            <div className="p-6">
              ...
              <div className="border-t border-gray-100 dark:border-gray-700 pt-4 mb-8">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">₹{total.toFixed(2)}</span>
                </div>
            ...
            <div className="bg-emerald-50 dark:bg-emerald-900/20 p-4 flex gap-3 text-sm text-emerald-800 dark:text-emerald-300">
              ...
              <p>By purchasing, you are directly supporting local farmers and sustainable agriculture.</p>
            </div>
          </div>
        </div>
      </div>
