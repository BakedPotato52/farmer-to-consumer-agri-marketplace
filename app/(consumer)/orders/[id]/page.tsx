...
export default async function OrderDetailPage(props: {
  ...
  return (
    <div className="max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      {/* Breadcrumb */}
      <div className="mb-6">
        <Link href="/orders" className="text-emerald-600 dark:text-emerald-400 hover:underline flex items-center gap-2 text-sm font-medium">
          ← Back to Orders
        </Link>
      </div>
          ...
          <div className="min-w-[600px]">
            <div className="flex justify-between items-center relative">
              ...
              <div 
                ...
                style={{ width: `${(Math.max(0, currentStatusIndex) / (ORDER_STATUS_FLOW.length - 1)) * 100}%` }}
              ></div>
                ...
                return (
                  <div key={status} className="relative z-10 flex flex-col items-center gap-2">
                    ...
                    <span className={`text-xs font-bold uppercase tracking-wider ${
                      isCompleted ? "text-emerald-700 dark:text-emerald-400" : "text-gray-400"
                    }`}>
                      {status}
                    </span>
                  </div>
      ...
      {order.status === "cancelled" && (
        <div className="bg-red-50 dark:bg-red-900/20 rounded-2xl p-6 border border-red-100 dark:border-red-900/50 mb-8 flex items-center gap-4">
          ...
          <div>
            <h3 className="font-bold text-red-800 dark:text-red-400 text-lg">Order Cancelled</h3>
            <p className="text-red-600 dark:text-red-300">This order has been cancelled.</p>
          </div>
        </div>
      )}
              ...
              {order.items.map((item) => (
                <div key={item.productId} className="p-6">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <Link href={`/products/${item.productId}`} className="font-bold text-gray-900 dark:text-white hover:text-emerald-600 transition-colors text-lg">
                        {item.productName}
                      </Link>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <div className="space-y-4">
              <div>
                <div className="text-xs text-gray-500 uppercase tracking-wider font-bold mb-1">Address</div>
                <div className="text-sm text-gray-900 dark:text-gray-300">{order.deliveryAddress}</div>
              </div>
              
              <div>
                <div className="text-xs text-gray-500 uppercase tracking-wider font-bold mb-1">Date & Slot</div>
              ...
              {order.notes && (
                <div>
                  <div className="text-xs text-gray-500 uppercase tracking-wider font-bold mb-1">Notes</div>
                  <div className="text-sm text-gray-900 dark:text-gray-300 italic">{order.notes}</div>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Farmer Details */}
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Farmer Details</h2>
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <div className="font-bold text-gray-900 dark:text-white">{order.farmerName}</div>
                <Link href={`/farmers/${order.farmerId}`} className="text-sm text-emerald-600 hover:underline">
                  View Profile
                </Link>
              </div>
            </div>
