      ...
      <div className="flex items-center justify-between">
        ...
        {nextStatus && (
          <form action={async () => {
            ...
            <button type="submit" className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-lg font-medium transition-colors shadow-sm">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <ul className="divide-y divide-gray-100">
              {order.items.map((item, idx) => (
                <li key={idx} className="p-6 flex items-center justify-between hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{item.quantity} {item.unit} × ₹{item.pricePerUnit}</p>
                    </div>
                  </div>
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        {index < ORDER_STATUS_FLOW.length - 1 && (
                          <div className={`w-0.5 h-full my-1 ${
                            ORDER_STATUS_FLOW.indexOf(order.status) > index ? 'bg-emerald-500' : 'bg-gray-200'
                          }`}></div>
                        )}
                      </div>
                      <div className="pb-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              {order.notes && (
                <div>
                  <p className="text-sm text-gray-500 font-medium">Order Notes</p>
                  <p className="text-gray-900 italic bg-amber-50 p-3 rounded-lg text-sm mt-1">{order.notes}</p>
                </div>
              )}
            </div>
          </div>
