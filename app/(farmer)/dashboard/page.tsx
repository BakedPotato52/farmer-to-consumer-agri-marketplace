    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="glass-card organic-shadow rounded-2xl p-6 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="mt-4">
            <p className="text-on-surface-variant text-xs font-bold uppercase tracking-wider">Total Sales</p>
        ...
        <div className="glass-card organic-shadow rounded-2xl p-6 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="mt-4">
            <p className="text-on-surface-variant text-xs font-bold uppercase tracking-wider">Total Products</p>
            <h3 className="font-heading text-2xl font-extrabold text-primary mt-1">
        ...
        <div className="glass-card organic-shadow rounded-2xl p-6 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="mt-4">
            <p className="text-on-surface-variant text-xs font-bold uppercase tracking-wider">Average Rating</p>
            <h3 className="font-heading text-2xl font-extrabold text-primary mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card organic-shadow rounded-3xl p-8 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h4 className="font-heading text-xl font-bold text-primary">Sales Performance</h4>
              <p className="text-on-surface-variant text-xs font-medium">Last 7 days revenue trend</p>
            </div>
          ...
          <div className="flex-1 flex items-end justify-between gap-4 min-h-50 pt-4">
            {chartData.map((item) => (
              <div key={item.day} className="flex flex-col items-center gap-3 w-full h-full justify-end">
                ...
                />
                <span className={`text-xs ${item.active ? "font-bold text-primary" : "text-outline"}`}>
                  {item.day}
                </span>
              </div>
            ))}
        ...
        <div className="space-y-6">
          <div className="glass-card organic-shadow rounded-3xl p-6 space-y-4">
            <h4 className="text-xs font-bold text-outline uppercase tracking-wider mb-2">Quick Actions</h4>
            <Link
              ...
              {recentOrders.length === 0 ? (
                ...
                recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-surface-container-low/50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-semibold">{order.consumerName}</td>
                    <td className="px-6 py-4 text-xs text-on-surface-variant">{order.items.length} items</td>
