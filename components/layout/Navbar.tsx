...
import { MdEco, MdHome, MdLogout, MdOutlineDashboard } from "react-icons/md";
...
export default function Navbar({
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b border-outline-variant/10 ${isScrolled
          ? "bg-surface/95 backdrop-blur-xl shadow-md h-16"
          : "bg-surface/90 backdrop-blur-xl shadow-sm h-20"
          }`}
      >
        <div className="flex justify-between items-center h-full px-4 md:px-[40px] max-w-[1280px] mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/cart"
              className={`p-2.5 rounded-xl transition-all active:scale-95 relative ${pathname === "/cart"
                ? "bg-primary/10 text-primary"
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-error text-on-error text-[10px] font-bold min-w-5 h-5 px-1 rounded-full flex items-center justify-center shadow-md animate-pulse">
            ...
            {session ? (
              <div className="relative group">
                <button
                  className="flex items-center gap-2 p-1.5 pr-3 rounded-full bg-surface-container-low hover:bg-surface-container-high border border-outline-variant/20 transition-all cursor-pointer"
            ...
            {/* Mobile Menu Button */}
            <button
              className="md:hidden p-2.5 text-on-surface-variant hover:bg-primary/10 rounded-xl cursor-pointer active:scale-95 transition-all"
        ...
        {(pathname === "/products" || pathname === "/") && (
          <div className="border-t border-outline-variant/10 bg-surface-container-lowest/80 backdrop-blur-md hidden sm:block">
      ...
      {/* Accessible Mobile Navigation Drawer Portal */}
      {mobileMenuOpen && (
        <div
        ...
        >
          ...
          <div className="relative z-101 w-[85vw] max-w-sm h-100dvh bg-white shadow-2xl p-6 overflow-y-auto flex flex-col justify-between border-l border-outline-variant/20">
            <div>
              {/* Drawer Header */}
              <div className="flex justify-between items-center pb-5 mb-6 border-b border-outline-variant/15">
                <span className="text-xl font-bold font-heading text-primary flex items-center gap-2">
                  <MdEco /> FarmFresh
                </span>
                <button
                  onClick={() => setMobileMenuOpen(false)}
                  className="p-2.5 text-on-surface-variant hover:bg-surface-container-high rounded-full transition-colors cursor-pointer"
                  aria-label="Close mobile navigation menu"
                >
                  <AiOutlineClose className="text-xl text-on-surface" />
                </button>
              </div>

              {/* Search Form */}
              <form onSubmit={handleSearchSubmit} className="mb-6">
                <div className="flex items-center bg-surface-container-low rounded-2xl px-4 py-3 border border-outline-variant/30 focus-within:ring-2 focus-within:ring-primary/20">
                  <IoIosSearch className="text-on-surface-variant text-xl mr-2 shrink-0" />
                  <input
                    type="text"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="Search fresh produce..."
                    className="w-full text-sm bg-transparent outline-none text-on-surface placeholder:text-outline"
                  />
                </div>
              </form>
              ...
              <div className="space-y-1.5 mb-6">
                <span className="text-[11px] font-bold text-outline uppercase tracking-wider px-3 block mb-2">
                  Main Navigation
                </span>
                {[
                  ...
                  return (
                    <Link
                      ...
                      href={item.href}
                      onClick={() => setMobileMenuOpen(false)}
                      className={`flex items-center justify-between px-4 py-3.5 rounded-2xl text-sm font-bold transition-all ${isActive
                        ? "bg-primary text-on-primary shadow-sm"
                        : "text-on-surface hover:bg-surface-container-high bg-surface-container-low/50"
                        }`}
                    >
                      <div className="flex items-center gap-3">
                      ...
                      {item.badge ? (
                        <span
                          className={`text-xs font-extrabold px-2.5 py-0.5 rounded-full ${isActive
              ...
              <div className="space-y-2">
                <span className="text-[11px] font-bold text-outline uppercase tracking-wider px-3 block mb-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {CATEGORIES_NAV.slice(1).map((cat) => (
                    <Link
                      ...
                      onClick={() => setMobileMenuOpen(false)}
                      className="p-3 bg-surface-container-low rounded-xl text-xs font-bold text-on-surface flex items-center gap-2 hover:bg-primary/10 transition-colors border border-outline-variant/10"
                    >
            ...
            <div className="pt-6 mt-6 border-t border-outline-variant/20 space-y-3">
              {session ? (
                <>
                  <div className="px-1">
                    <div className="text-xs text-outline">Signed in as</div>
                    <div className="text-sm font-bold text-on-surface truncate">
                      {session.name}
                    </div>
                  </div>
                  <Link
                    ...
                    onClick={() => setMobileMenuOpen(false)}
                    className="block w-full text-center px-4 py-3 bg-primary text-on-primary rounded-xl text-sm font-bold shadow-md hover:bg-primary-container transition-all"
                  >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Link
                    href="/login"
                    onClick={() => setMobileMenuOpen(false)}
                      className="text-center px-4 py-3 border border-outline-variant/30 text-primary rounded-xl text-xs font-bold hover:bg-primary/5 transition-all"
                  >
                    Log in
                  </Link>
                  <Link
                    href="/register"
                    onClick={() => setMobileMenuOpen(false)}
                      className="text-center px-4 py-3 bg-primary text-on-primary rounded-xl text-xs font-bold shadow-md hover:bg-primary-container transition-all"
                  >
                    Sign up
                  </Link>
                </div>
